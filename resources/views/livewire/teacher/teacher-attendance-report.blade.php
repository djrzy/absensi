    ...
    @if ($availableClassrooms->isEmpty())
        ...
        <div class="bg-white p-8 sm:p-12 rounded-2xl border border-gray-100 shadow-xs text-center space-y-3">
            ...
            <p class="text-xs text-gray-500 max-w-md mx-auto">
                Anda belum terdaftar mengampu mata pelajaran di kelas manapun. Silakan hubungi tim Kurikulum atau
                Administrator untuk pengaturan jadwal mapel.
            </p>
        </div>
    @else
        <!-- KOP CETAK DOKUMEN RESMI (PRINT ONLY) -->
        <div class="print-only mb-6">
            ...
            <div class="text-center my-4 space-y-1">
                <h3 class="text-base font-bold uppercase underline tracking-wider">LAPORAN REKAPITULASI PRESENSI MATA
                    PELAJARAN</h3>
            ...
            <table class="w-full text-[9pt] mb-4 border-none !border-0">
                <tr class="!border-0">
                    <td class="w-28 font-bold !border-0 p-1">Mata Pelajaran</td>
                    <td class="w-4 !border-0 p-1">:</td>
                    <td class="!border-0 p-1"><strong>{{ $selectedSubject->name ?? '-' }}</strong></td>
                    <td class="w-28 font-bold !border-0 p-1">Guru Pengampu</td>
                    <td class="w-4 !border-0 p-1">:</td>
                    <td class="!border-0 p-1">{{ auth()->user()->name }}</td>
                </tr>
                <tr class="!border-0">
                    <td class="font-bold !border-0 p-1">Kelas</td>
        ...
        <!-- HEADER BANNER & DEPENDENT DROPDOWN (NO PRINT) -->
        <div
            ...
            <div class="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
                ...
                <!-- DROPDOWN 1: PILIHAN KELAS -->
                <div class="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl px-2.5 py-1 text-xs">
                    <span class="text-gray-400 font-bold">🏫 Kelas:</span>
                    <select wire:model.live="selectedClassroomId"
                        class="bg-transparent font-bold text-gray-800 outline-none cursor-pointer">
                ...
                <div class="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl px-2.5 py-1 text-xs">
                    ...
                    <select wire:model.live="selectedSubjectId"
                        class="bg-transparent font-bold text-indigo-900 outline-none cursor-pointer"
                        {{ $availableSubjects->isEmpty() ? 'disabled' : '' }}>
                        @forelse ($availableSubjects as $sbj)
        ...
        <div class="print-only pt-10">
            <div class="flex justify-between text-[10pt] text-center font-medium">
                ...
                <div class="space-y-16">
                    <p>Bandung, {{ now()->locale('id')->translatedFormat('d F Y') }}<br>Guru Mata Pelajaran</p>
