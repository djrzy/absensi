    ...
    <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs h-fit">
        ...
        <form wire:submit.prevent="store" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model="description" placeholder="Misal: Libur Tahun Baru Baru"
                    class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all">
                @error('description')
                    <span class="text-xs text-rose-600 mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <button type="submit"
                class="w-full py-2.5 bg-gray-900 hover:bg-gray-800 text-white text-xs font-bold rounded-xl transition-all cursor-pointer shadow-xs">
                Simpan Hari Libur
            </button>
        </form>
    </div>
    ...
    <div class="md:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm text-gray-700">
                @if ($holidays->isEmpty())
                    <tr>
                        <td colspan="3" class="p-8 text-center text-gray-400 text-xs">Belum ada agenda libur yang
                            didaftarkan.</td>
                    </tr>
                @else
