<div class="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form wire:submit.prevent="store" class="space-y-3">
            <div>
                ...
                <select wire:model="classroom_id"
                    class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all">
                ...
                <div class="grid grid-cols-2 gap-1.5">
                    <div>
                        ...
                        <input type="number" wire:model="period_start" placeholder="1"
                            class="w-full bg-gray-50 border border-gray-200 rounded-xl px-2 py-2 text-xs text-center focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none transition-all">
                    </div>
                    <div>
            ...
            <button type="submit"
                class="w-full mt-2 py-2.5 bg-gray-900 hover:bg-gray-800 text-white text-xs font-bold rounded-xl transition-all cursor-pointer shadow-xs">
                Simpan Jadwal
            </button>
        </form>
    </div>
    ...
    <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-xs overflow-hidden h-fit">
        <div
            class="p-4 bg-gray-50/50 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <select wire:model.live="filter_classroom_id"
                class="bg-white border border-gray-200 rounded-xl px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-blue-100">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50 text-xs text-gray-700">
                    @if ($schedules->isEmpty())
                        <tr>
                            <td colspan="5" class="p-8 text-center text-gray-400">Belum ada jadwal yang di-generate.
                            </td>
                        </tr>
                    @else
                        @foreach ($schedules as $sched)
                            <tr class="hover:bg-gray-50/30 transition-colors">
                                <td class="p-4">
                                    ...
                                    <span class="text-gray-400 font-mono">Jam
                                        {{ $sched->period_start }}-{{ $sched->period_end }}</span>
                                </td>
                                <td class="p-4 font-semibold text-gray-800">{{ $sched->classroom->name }}</td>
                                <td class="p-4">
                                    <span class="font-medium text-gray-900 block">{{ $sched->subject->name }}</span>
                                    <span
                                        class="text-gray-400 text-[10px] bg-gray-100 px-1.5 py-0.5 rounded">{{ $sched->subject->code }}</span>
                                </td>
                                <td class="p-4 text-gray-600">{{ $sched->teacher->name }}</td>
                                <td class="p-4 text-center">
                                    <button wire:click="delete({{ $sched->id }})"
