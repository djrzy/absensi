<div class="p-6 max-w-7xl mx-auto space-y-6" x-data="{
    ...
    <!-- Ganti bagian <style> di paling atas resources/views/livewire/admin/attendance-report.blade.php -->
    <style>
        @media print {
            @page {
                size: A4 landscape;
            ...
            /* SEMBUNYIKAN NAVBAR / SIDEBAR GLOBAL APLIKASI */
            nav,
            header,
            sidebar,
            aside,
    ...
    <!-- MODAL DETAIL AUDIT TRAIL (NO PRINT) -->
    ...
    <!-- KOP DINAS / LAPORAN RESMI (HANYA MUNCUL SAAT CETAK / SAVE PDF) -->
    <div class="print-only mb-6">
        ...
        <div class="text-center pb-3 border-b-2 border-black space-y-1 relative">
            ...
            <h1 class="text-xl font-extrabold uppercase tracking-wide">SEKOLAH MENENGAH PERTAMA (SMP) NEGERI KHAYALAN
            </h1>
            <p class="text-[10pt] italic">Jl. Pendidikan No. 45, Kota Mimpi, Jawa Barat • Telp: (021) 1234567 •
                Website: www.school.id</p>
            <div class="border-b-4 border-black mt-1"></div>
        </div>

        <!-- Judul Dokumen -->
        <div class="text-center my-4 space-y-1">
            <h3 class="text-base font-bold uppercase underline tracking-wider">LAPORAN REKAPITULASI PRESENSI SISWA</h3>
            <p class="text-[10pt] font-semibold">PERIODE: {{ strtoupper($periodeText) }}</p>
        </div>

        <!-- Meta Info Informasi Laporan -->
        <table class="w-full text-[9pt] mb-4 border-none !border-0">
            <tr class="!border-0">
                ...
                <td class="!border-0 p-1"><strong>{{ $selectedClassroom->name ?? '-' }}</strong></td>
                <td class="w-32 font-bold !border-0 p-1">Tahun Ajaran</td>
                <td class="w-4 !border-0 p-1">:</td>
            ...
            <tr class="!border-0">
                ...
                <td class="!border-0 p-1">{{ $selectedClassroom->waliKelas->name ?? 'Belum Ditentukan' }}</td>
                <td class="font-bold !border-0 p-1">Tanggal Cetak</td>
                <td class="!border-0 p-1">:</td>
    ...
    <!-- HEADER BANNER & FILTER INTERAKTIF (NO PRINT) -->
    <div
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">Rekapitulasi Absensi (Rentang Bulan)</h1>
            <p class="text-xs text-gray-500 mt-0.5">Filter kumulatif rentang bulan dengan ekspor Excel/PDF resmi.</p>
        </div>

        <div class="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
            <!-- Filter Dropdown Kelas -->
            <select wire:model.live="selectedClassroomId"
                class="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-xs font-medium outline-none focus:ring-2 focus:ring-indigo-100 focus:bg-white transition-all cursor-pointer">
                @foreach ($classrooms as $class)
                    <option value="{{ $class->id }}">Kelas: {{ $class->name }}</option>
                @endforeach
            </select>

            <!-- CUSTOM MONTH-YEAR RANGE PICKER (POPOVER DROPDOWN) -->
            <div class="relative">
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Sampai Bulan
                                (YYYY-MM):</label>
                            <input type="month" wire:model.live.debounce.500ms="endMonth" pattern="[0-9]{4}-[0-9]{2}"
                                maxlength="7" placeholder="2026-12"
                                class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5 text-xs font-medium outline-none focus:ring-2 focus:ring-indigo-100 focus:bg-white transition-all">
                        </div>
                    </div>
    ...
    <!-- TABEL RINGKAS KUMULATIF RENTANG BULAN -->
            ...
            <tbody class="divide-y divide-gray-50 text-xs text-gray-700">
                @forelse ($reportData as $index => $studentData)
                    <tr class="hover:bg-indigo-50/30 transition-colors cursor-pointer"
                        ...
                        <td class="p-4 text-center font-bold text-emerald-600">{{ $studentData['summary']['Hadir'] }}
                        </td>
                        <td class="p-4 text-center font-bold text-indigo-600">
                            {{ $studentData['summary']['Terlambat'] }}</td>
                        <td class="p-4 text-center font-bold text-amber-600">{{ $studentData['summary']['Sakit'] }}
                        </td>
                        <td class="p-4 text-center font-bold text-blue-600">{{ $studentData['summary']['Izin'] }}</td>
                        <td class="p-4 text-center font-bold text-rose-600">{{ $studentData['summary']['Alpa'] }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="10" class="p-8 text-center text-gray-400">Belum ada data presensi pada rentang
                            bulan ini.</td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="print-only pt-10">
        <div class="flex justify-between text-[10pt] text-center font-medium">
            <div class="space-y-16">
                <p>Mengetahui,<br>Kepala SMP Negeri Khayalan</p>
            ...
            <div class="space-y-16">
                <p>Jakarta, {{ now()->locale('id')->translatedFormat('d F Y') }}<br>Wali Kelas
                    {{ $selectedClassroom->name ?? '' }}</p>
                <p class="font-bold underline uppercase">(
                    {{ $selectedClassroom->waliKelas->name ?? '....................................................' }}
                    )<br><span class="no-underline font-normal text-[9pt]">NIP. -</span></p>
            </div>
        </div>
    </div>

    <!-- SIDE DRAWER PANEL (NO PRINT) -->
    <div x-show="openDrawer" class="fixed inset-0 z-50 overflow-hidden no-print" style="display: none;">
        <div class="absolute inset-0 bg-gray-900/40 backdrop-blur-xs transition-opacity" @click="openDrawer = false">
        </div>
        ...
        <div class="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <div class="w-screen max-w-lg bg-white shadow-2xl flex flex-col justify-between">
                ...
                <div class="p-5 overflow-y-auto space-y-5 flex-1">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Pilih Bulan Yang Ingin
                            Dilihat:</label>
                        <select x-model="selectedMonthKey"
                            ...
                            <template x-for="(monthData, key) in drawerData.monthly_breakdown" :key="key">
                    ...
                    <!-- KALENDER GRID -->
                    <template x-if="drawerData.monthly_breakdown && drawerData.monthly_breakdown[selectedMonthKey]">
                        <div>
                            <div class="flex justify-between items-center mb-3">
                                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider"
                                    x-text="'Matriks: ' + drawerData.monthly_breakdown[selectedMonthKey].label"></h4>
                                <span class="text-[10px] text-indigo-600 font-semibold">💡 Klik tanggal untuk detail
                                    mapel</span>
                            </div>
                                ...
                                <template
                                    ...
                                    <div>
                                        <template x-if="cell.is_empty">
                                            <div class="w-full p-2 rounded-xl border border-transparent min-h-[52px]">
                                            </div>
                                        </template>
